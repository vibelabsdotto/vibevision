  ...
  return (
    <div className="space-y-6">
      ...
      <section className={`${surfaceClasses} overflow-hidden`}>
        <div className="grid gap-6 px-5 py-6 sm:px-7 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,0.9fr)] lg:items-end">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-ink-2 sm:text-base">
          ...
          <div className="rounded-[20px] border border-teal/25 bg-teal/10 p-5">
            <p className="eyebrow">This week</p>
            <div className="mt-3 flex items-end justify-between gap-3">
              <div>
                <p className="font-display text-4xl font-bold tracking-tight">{formatPercent(data.score.weeklyScore)}</p>
                <p className="mt-1 text-sm text-ink-2">Execution score</p>
              </div>
              <p className="text-sm text-ink-3">Week {data.currentWeek} of 12</p>
            </div>
            <div className="mt-4">
      ...
      <section className="grid grid-cols-2 gap-4 xl:grid-cols-4">
        <MetricCard label="Week" value={`${data.currentWeek}/12`} />
        <MetricCard detail="Including today" label="Days left" value={`${data.daysLeft}`} />
        <MetricCard
          detail={`${data.todaySummary.remainingCount} tactics still open`}
          label="Today complete"
          value={`${data.todaySummary.completedCount}/${data.todaySummary.relevantCount}`}
        />
        <MetricCard detail="Remaining across due tactics today" label="Outstanding units" value={data.todaySummary.totalRemaining.toFixed(1)} />
      </section>

      {/* goals + today */}
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(0,0.9fr)]">
        <div className={`${surfaceClasses} p-5 sm:p-7`}>
          <SectionHeader
            eyebrow="Current goal health, lag indicators, and weekly momentum."
            href={`/cycles/${data.cycle.id}/goals`}
            label="All goals"
            title="Goals"
          />
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        ...
        <div className={`${surfaceClasses} p-5 sm:p-7`}>
          <SectionHeader eyebrow="Jump straight into a specific week." href={`/cycles/${data.cycle.id}`} label="Cycle detail" title="Cycle weeks" />
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {data.weeks.map((week) => {
              const review = data.reviews.find((item) => item.weekNumber === week.weekNumber);
              return (
                <Link
                  className="flex items-start justify-between gap-3 rounded-[16px] border border-border bg-surface-2/50 p-4 transition hover:border-coral/50"
                  href={`/cycles/${data.cycle.id}/week/${week.weekNumber}`}
                  key={week.id}
                >
                  <div>
                  ...
                  <span
                    className={`rounded-full px-2.5 py-1 font-mono text-[11px] tracking-[0.14em] uppercase ${
                      review ? "bg-teal/15 text-teal" : "bg-surface-2 text-ink-2"
                    }`}
                  >
